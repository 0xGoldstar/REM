...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // 0-based index of the currently selected video in a multi-video post
    private val _selectedEntryIndex = MutableStateFlow(0)
    val selectedEntryIndex: StateFlow<Int> = _selectedEntryIndex.asStateFlow()
    ...
    /** Switch the active video in a multi-video post (index is 0-based). */
    fun selectEntry(index: Int) {
        if (_selectedEntryIndex.value == index) return
        _selectedEntryIndex.value = index
        val url = (_uiState.value as? UiState.VideoInfoLoaded)?.info?.sourceUrl ?: return
        cancelPreview()
        _previewState.value = PreviewState.Idle
        resetEditState()
        // playlistIndex is 1-based for yt-dlp
        startPreviewDownload(url, playlistIndex = index + 1)
    }

    private fun startPreviewDownload(url: String, playlistIndex: Int = 0) {
    ...
    fun startDownload(options: DownloadOptions) {
        ...
        val inputData = Data.Builder()
            ...
            .putInt(DownloadWorker.KEY_PLAYLIST_INDEX, options.playlistIndex)
            // Edit config
    ...
    fun estimateGifSize(): String {
        ...
        return MediaUtils.formatFileSize(estimatedBytes.coerceAtLeast(100_000))
    }
