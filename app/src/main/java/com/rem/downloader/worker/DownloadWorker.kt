...
class DownloadWorker(
    ...
    companion object {
        ...
        // 1-based index into a multi-video post; 0 = no specific index
        const val KEY_PLAYLIST_INDEX = "playlist_index"
    ...
    override suspend fun doWork(): Result {
        ...
        val playlistIndex = inputData.getInt(KEY_PLAYLIST_INDEX, 0)
        ...
        return if (editConfig.hasEdits) {
            downloadWithEdits(url, format, resolution, customFilename, outputDir, ytDlp, editConfig, cropRefWidth, cropRefHeight, playlistIndex)
        } else {
            downloadDirect(url, format, resolution, customFilename, outputDir, ytDlp, playlistIndex)
        }
    }
